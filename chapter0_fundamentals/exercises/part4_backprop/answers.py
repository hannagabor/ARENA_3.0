...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    broadcasted: An array that was formerly of the same shape of 'original' and was expanded by
        broadcasting rules.
    """
    broadcasted_shape = broadcasted.shape
    original_shape = original.shape
    num_dim_diff = len(broadcasted_shape) - len(original_shape)
    intermediate_shape = [1 for _ in range(num_dim_diff)] + list(original_shape)

    axis_to_sum_over = []
    for i, (b_dim, i_dim) in enumerate(zip(broadcasted_shape, intermediate_shape)):
        if b_dim > i_dim:
            axis_to_sum_over.append(i)
    broadcasted = broadcasted.sum(axis=tuple(axis_to_sum_over), keepdims=True)
    broadcasted = broadcasted.squeeze(axis=tuple(range(num_dim_diff)))
...
def forward_and_back(a: Arr, b: Arr, c: Arr) -> tuple[Arr, Arr, Arr]:
    ...
    dg_da = multiply_back0(dd, d, a, b)
    dg_db = multiply_back1(dd, d, a, b)
...
def multiply_forward(a: Tensor | int, b: Tensor | int) -> Tensor:
    ...
    out = Tensor(arg_a * arg_b, requires_grad)
    if requires_grad:
        parents = {}
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = (
            ...
            and any(t.requires_grad for t in args if isinstance(t, Tensor))
        )
        out = Tensor(numpy_func(*arg_arrays, **kwargs), requires_grad)
        parents = {i: arg for i, arg in enumerate(args) if isinstance(arg, Tensor)}
        # tensor_args_values = tuple(t.array for t in tensor_args)
        if requires_grad:
            out.recipe = Recipe(
                numpy_func,
                arg_arrays,
...
def topological_sort(node: Node, get_children: Callable) -> list[Node]:
    ...
    while stack:
        ...
        if next in visited:
            if next not in left:
                topological_order.append(next)
                left.add(next)
            continue
        stack.append(next)
        visited.add(next)
        for child in get_children(next):
            if child in visited:
                if child in ancestors[next]:
                    raise ValueError("Graph is not acyclic")
                continue
            child_ancestors = ancestors[next].copy()
            child_ancestors.add(next)
...
def backprop(end_node: Tensor, end_grad: Tensor | None = None) -> None:
    ...
    for node in nodes:
        ...
        if recipe is None:
            continue
        for i, parent in recipe.parents.items():
            back_func = BACK_FUNCS.get_back_func(recipe.func, i)
            i_grad = back_func(grad, node.array, *node.recipe.args, **node.recipe.kwargs)
            if parent in grads:
                print("in")
                grads[parent] += i_grad
            else:
...
def sum_back(grad_out: Arr, out: Arr, x: Arr, dim=None, keepdim=False):
    ...
    if not keepdim:
        if dim is None:
            return np.ones(x.shape) * grad_out
        else:
            out = np.expand_dims(grad_out, dim)
    else:
        out = grad_out
    return np.broadcast_to(out, x.shape)
...
def maximum_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    ...
    grad = np.where(x > y, grad_out, np.zeros_like(grad_out))
    return unbroadcast(np.where(x == y, 0.5 * grad_out, grad), x)


def maximum_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    """Backwards function for max(x, y) wrt y."""
    grad = np.where(x > y, np.zeros_like(grad_out), grad_out)
    return unbroadcast(np.where(x == y, 0.5 * grad_out, grad), y)


def relu(x: Tensor) -> Tensor:
    """Like torch.nn.function.relu(x, inplace=False)."""
    return maximum(Tensor(0), x)
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        upper = 1 / sqrt(in_features)
        lower = -upper
        self.weight = Parameter(
            Tensor(np.random.uniform(lower, upper, (out_features, in_features)))
        )
        if bias:
            self.bias = Parameter(Tensor(np.random.uniform(lower, upper, out_features)))
        else:
            self.bias = None
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    exps = exp(logits)
    sum_of_exps = sum(exps, dim=1)
    exp_for_true_classes = exps[arange(0, batch_size), true_labels]
    return -log(exp_for_true_classes / sum_of_exps)
